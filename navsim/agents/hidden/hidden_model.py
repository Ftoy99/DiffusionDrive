...
from navsim.agents.hidden.modules.blocks import linear_relu_ln, bias_init_with_prob, gen_sineembed_for_position, \
...
class HiddenModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor] = None) -> Dict[
        ...
        qformer_q = self._gaze_embedding.unsqueeze(0).expand(gaze_tokens_flat.shape[0], -1, -1).permute(0,2,1)  # [64, 5, 256]
