...
from navsim.agents.diffusiondrive.modules.blocks import linear_relu_ln, bias_init_with_prob, gen_sineembed_for_position, \
...
class V2TransfuserModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor] = None) -> Dict[
        ...
        print(f"cross_bev_feature.shape after bev_proj {cross_bev_feature.shape}")
        cross_bev_feature = cross_bev_feature.permute(0, 2, 1).contiguous().view(batch_size, -1, bev_spatial_shape[0],
                                                                                 bev_spatial_shape[1])
        print(f"cross_bev_feature.shape after permute {cross_bev_feature.shape}")
