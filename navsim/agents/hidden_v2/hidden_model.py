...
from navsim.agents.hidden_v2.transfuser_backbone import TransfuserBackbone
from navsim.agents.hidden_v2.hidden_features import BoundingBox2DIndex
from navsim.common.enums import StateSE2Index
from diffusers.schedulers import DDIMScheduler
from navsim.agents.hidden_v2.modules.conditional_unet1d import SinusoidalPosEmb
import torch.nn.functional as F
from navsim.agents.hidden_v2.modules.blocks import linear_relu_ln, bias_init_with_prob, gen_sineembed_for_position, \
    GridSampleCrossBEVAttention
from navsim.agents.hidden_v2.modules.multimodal_loss import LossComputer
...
class RMSNorm(nn.Module):
    def __init__(self, d, eps: float = 1e-8):
        super().__init__()
        self.eps = eps
...
class HiddenModel(nn.Module):
    ...
    def __init__(self, config: HiddenConfig):
        ...
        self._gaze_embedding = nn.Embedding(32, config.tf_d_model)  # 5 x D
...
class TrajectoryHead(nn.Module):
    ...
    def forward_train(self, ego_query, agents_query, bev_feature, bev_spatial_shape, status_encoding, gaze_query,
        ...
        mask = zero_mask.all(dim=-1)  # [B, neighbors, modes] → True where trajectory is all zero
        print(f"mask {mask}")
